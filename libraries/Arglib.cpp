...
void ArduboyAudio::tone(uint8_t channel, unsigned int frequency, unsigned long duration) {
  sdl_beep(channel, frequency, duration);
}
...
void ArduboyTunes::step() {
  ...
  while (1) {
    command = pgm_read_byte((const uint8_t*)score_cursor++);
    opcode = command & 0xf0;
    chan = command & 0x0f;
    if (opcode == TUNE_OP_STOPNOTE) { /* stop note */
      ...
      playNote(chan, pgm_read_byte((const uint8_t*)score_cursor++));
    }
      ...
      duration = ((unsigned)command << 8) | (pgm_read_byte((const uint8_t*)score_cursor++));
...
void ArduboyTunes::tone(unsigned int frequency, unsigned long duration) {
  sdl_beep(0, frequency, duration);
  sdl_beep(1, frequency, duration);
}
